...
class CampaignContextManager {
    ...
    extractCharacterProfiles() {
        ...
        // Determine genders based on pronoun counts
        for (const [name, pronouns] of Object.entries(characters)) {
            const femaleCount = pronouns.she + pronouns.her;
            const maleCount = pronouns.he + pronouns.his + pronouns.him;

            this.characterProfiles[name] = {
                gender: femaleCount > maleCount ? 'female' : 'male',
                pronouns: femaleCount > maleCount ? 'she/her' : 'he/him',
                evidenceCount: femaleCount + maleCount,
                breakdown: { female: femaleCount, male: maleCount }
            };
        }
    }
...
Remember: You are continuing an established story. Reference past events, maintain character relationships, and progress the narrative consistently.`;
    ...
    buildCampaignContext() {
        ...
        return `## CURRENT CAMPAIGN CONTEXT:
...
Remember: You are continuing an established story. Reference past events, maintain character relationships, and progress the narrative consistently.`;
    ...
    formatPartyInfo() {
        ...
        for (const [key, member] of Object.entries(this.campaignState.party)) {
            ...
            info += `
**${member.name}** (${member.race_class || member.background})${genderInfo}
- Credits: ${member.credits}
- Key traits: ${member.personality}
- Current status: ${member.hp ? `${member.hp.current}/${member.hp.max} HP` : 'Healthy'}`;
        }
        return info;
    }
