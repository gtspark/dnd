...
class ClaudeAPIHandler {
    ...
    async initialize() {
        try {
            ...
            window.__CLAUDE_API_CONFIG__ = this.config;
    ...
    updateSandboxUI() {
        ...
        title.textContent = 'Titan Station Crisis';
    }
    ...
    async sendMessage(playerMessage, mode = 'ic', isRollResult = false) {
        ...
        try {
            ...
            // Preserve metadata from raw response for downstream consumers
            ...
            parsedResponse.combatState = response.combatState;
            parsedResponse.campaignState = response.campaignState;
            parsedResponse.contextActive = response.contextActive;
            parsedResponse.contextStats = response.contextStats;

            if (response.campaignState) {
                this.campaignState = response.campaignState;
            }
    ...
    async callClaudeAPI(message, mode = 'ic', isRollResult = false) {
        const primaryBase = this.config?.base_url || '/api/dnd';
        const fallbackBase = this.config?.fallback_base_url || '/dnd-api/dnd';
        const candidateBases = Array.from(new Set([primaryBase, fallbackBase, '/dnd/api/dnd', '/dnd-api/dnd', '/api/dnd'].filter(Boolean)));
        ...
        console.info('🔁 Enhanced server endpoint candidates:', candidateBases);

        for (const base of candidateBases) {
            const apiUrl = `${base.replace(/\/$/, '')}/action`;
            try {
                console.info(`🛰️ Attempting DM request via ${apiUrl}`);
                ...
                return {
                    ...
                    contextActive: data.contextActive,
                    contextStats: data.contextStats
                };
            } catch (error) {
                console.warn(`Call to ${apiUrl} failed:`, error?.message || error);
                lastError = error;
            }
        }

        throw new Error(`Enhanced server failed: ${lastError?.message || 'no reachable endpoint'}`);
    }
    ...
    parseRollRequest(rollRequest) {
        // Parse "Roll Technology/Hacking (DC 15) to attempt something" or "Roll Intelligence to decipher"
        const match = rollRequest.match(/Roll\s+([^(]+?)(?:\s*\(DC\s*(\d+)\))?\s*to\s*(.+)/i);
